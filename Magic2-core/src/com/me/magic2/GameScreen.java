...
public class GameScreen extends BaseScreen{
	...
	public GameScreen(MagicMain rm) {
		...
		lang.addListener(new ClickListener(){
			public void clicked (InputEvent event, float x, float y) {
				Skin s =  getGame().getManager().getSkin();
				
				final Dialog langDialog =new Dialog("",s, "default");
				...
				lang_ru.addListener(new ClickListener(){
					public void clicked (InputEvent event, float x, float y) {
						String lang =  getGame().getL10n().getLanguage();
						if(!"ru".equals(lang)){
							getGame().getL10n().setLanguage("ru");
						}
						langDialog.hide();
					
						Settings s = getGame().getSettings();
						s.setLanguage("ru");
						s.write();
					}
				});
				...
				lang_en.addListener(new ClickListener(){
					public void clicked (InputEvent event, float x, float y) {
						...
						String lang =  getGame().getL10n().getLanguage();
						if(!"en".equals(lang)){
							getGame().getL10n().setLanguage("en");
						}
						langDialog.hide();
