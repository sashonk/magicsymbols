...
import com.me.magic2.IActivityRequestHandler.ConnectionStatusCallback;
...
public class GameScreen extends BaseScreen {
		...
		return new InputListener() {
			public boolean keyDown(InputEvent event, int keycode) {
				...
				if (keycode == Keys.BACK || keycode == Keys.ESCAPE) {
					...
					final com.badlogic.gdx.Preferences prefs = Gdx.app
							.getPreferences(getGame().getClass().getName());
					final boolean rated = prefs.getBoolean(Values.rated, false);
					final int exitClickCount = prefs.getInteger(
							Values.exit_click_count, 0);
					final boolean played = prefs.getBoolean(Values.played,
							false);

					if (!rated && played && (exitClickCount % 3 == 0)
							&& created) {
						getGame().getNative().checkInternetConnection(
									...
									public void online() {
										Gdx.app.postRunnable(new Runnable() {
											...
											public void run() {
												...
												Button yes = new L10nButton(
														getGame().getL10n(),
														"yes_rate", getGame()
																.getManager()
																.getSkin(),
														"small");
												Button no = new L10nButton(
														getGame().getL10n(),
														"no_rate", getGame()
																.getManager()
																.getSkin(),
					...
					if (played) {
						prefs.putInteger(Values.exit_click_count,
								exitClickCount + 1);
						prefs.flush();
					}
	...
	public GameScreen(MagicMain rm) {
		...
		final OrthographicCamera camera = (OrthographicCamera) getStage()
				.getCamera();
		detector = new GestureDetector(new GestureListener() {
			...
			public boolean pan(float x, float y, float deltaX, float deltaY) {
				...
				camera.position.add(-deltaX * camera.zoom,
						deltaY * camera.zoom, 0);
				constrainPosition(camera, getStage());
				constrainZoom(camera);
				return false;
			}
		...
		menu.add(begin).row();
		final Button lang = new L10nButton(rm.getL10n(), "language", skin,
				"huge");
		menu.add(lang).row();
		/*
		 * final Label glaga = new Label("abcdefghij", skin, "glaga");
		 * menu.add(glaga).row(); final Label glaga2 = new Label("klmnopqrs",
		 * skin, "glaga"); menu.add(glaga2).row(); final Label glaga3 = new
		 * Label("tuqvwxyz", skin, "glaga"); menu.add(glaga3).row();
		 */
	...
	void createGame() {
		...
		Label instruction1 = new L10nLabel(getGame().getL10n(), "line1", skin,
				"small");
		Label instruction2 = new L10nLabel(getGame().getL10n(), "line2", skin,
				"small");
		Label instruction3 = new L10nLabel(getGame().getL10n(), "line3", skin,
				"small");
		Label instruction4 = new L10nLabel(getGame().getL10n(), "line4", skin,
				"small");
		Label instruction5 = new L10nLabel(getGame().getL10n(), "line5", skin,
				"small");
		Label instruction6 = new L10nLabel(getGame().getL10n(), "line6", skin,
				"small");
		Label instruction7 = new L10nLabel(getGame().getL10n(), "line7", skin,
				"small");
		Label instruction8 = new L10nLabel(getGame().getL10n(), "line8", skin,
				"small");
		...
		for (int i = 1; i <= 10; i++) {
			...
			if (occurs != 10) {
				throw new IllegalStateException(String.format(
						"expected 10, but found %d", occurs));
			}
		...
		Image blackSquare = new Image(getGame().getManager().getAtlas()
				.findRegion("magic"));
		...
		blackSquare.addListener(new ClickListener() {
			public void clicked(InputEvent event, float x, float y) {
				...
				final OrthographicCamera cam = (OrthographicCamera) getStage()
						.getCamera();
				cam.zoom = 1;
				...
				Preferences preferences = Gdx.app.getPreferences(getGame()
						.getClass().getName());
				preferences.putBoolean(Values.played, true);
				preferences.flush();

				main.addAction(Actions.sequence(Actions.fadeOut(1),
						Actions.removeActor()));
						...
						Actions.sequence(Actions.delay(1),
								Actions.run(new Runnable() {
									...
									public void run() {
										...
										final String symbolOfInterest = symbolMap
												.get(valueOfInterest);
										// final Image spot = new
										// Image(getGame().getManager().getAtlas().findRegion(Integer.toString(valueOfInterest)));
										final Group spot = new Group();
										final Label spotLabel = new Label(
												symbolOfInterest, skin, "xsize");
										...
										spot.addAction(Actions.sequence(Actions
												.fadeIn(1)));
										float mul = 1;
										spot.setSize(spot.getWidth() * mul,
												spot.getHeight() * mul);
										Util.center(spot);

										// ///// RESTART //////////
										final TextButton restart = new L10nButton(
												getGame().getL10n(), "restart",
												getGame().getManager()
														.getSkin(), "medium");
										...
										int playsCount = prefs.getInteger(
												Values.playsCount, 1);
										prefs.putInteger(Values.playsCount,
												playsCount + 1);
										prefs.flush();
															...
															Actions.sequence(
																	...
																	Actions.run(new Runnable() {
																		...
																		public void run() {
																			...
																			TextButton guess = new L10nButton(
																					getGame()
																							.getL10n(),
																					"how_it_guess",
																					getGame()
																			...
																			TextButton howto = new L10nButton(
																					getGame()
																							.getL10n(),
																					"howto",
																					getGame()
																				...
																				public void clicked(
																					...
																					close.addListener(new ClickListener() {
																						public void clicked(
																								...
																								float x,
																								float y) {
																							final int interstitialShowCount = prefs
																									.getInteger(
																											Values.guess_click_count,
																											0);
																					...
																					Label superWantRate = new L10nLabel(
																							getGame()
																									.getL10n(),
																							"super_want_rate",
																							getGame()
																			...
																			float targetY = (getStage()
																					.getHeight()
																					+ getStage()
																							.getHeight()
																					/ 2 + spot
																					.getHeight() / 2) / 2 - 40;
																			uplabel.addAction(Actions
																					.sequence(Actions
																							.moveTo(uplabel
																									.getX(),
																									targetY,
	...
	void offlineDialog() {
		...
		final Dialog dialog = new Dialog("", getGame().getManager().getSkin(),
				"default");
		...
		dialog.setModal(true);
		dialog.setMovable(false);
		Label label = new L10nLabel(getGame().getL10n(), "internet", getGame()
				.getManager().getSkin(), "tiny");
		label.setAlignment(Align.center);
		label.setWrap(true);
		dialog.getContentTable().add(label).width(400).pad(15);

		Button close = new L10nButton(getGame().getL10n(), "ok", getGame()
				.getManager().getSkin(), "small");
		dialog.button(close);
		close.addListener(new ClickListener() {
			public void clicked(InputEvent event, float x, float y) {
				dialog.hide();
	...
	static Action blink() {
		return Actions.forever(Actions.sequence(Actions.fadeIn(0.3f),
